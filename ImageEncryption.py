...
def imageEncrypt(data, rsa_publickey, ecc_publickey):
    ...
    print "rsaEncrypt %s seconds" % (time.time() - start_time)
    rsa_encrypted = padding(rsa_encrypted,31)
    #ecc_encrypted = eccEncrypt(rsa_encrypted, ecc_publickey)
    start_time = time.time()
    ecc_encrypted = doEccEncrypt(rsa_encrypted, ecc_publickey)
    print "doEccEncrypt %s seconds" % (time.time() - start_time)
...
def imageDecrypt(data, rsa_privatekey, ecc_privatekey):
    ...
    start_time = time.time()
    ecc_decrypted = doEccDecrypt(data, ecc_privatekey)
    print "doEccDecrypt %s seconds" % (time.time() - start_time)
    ecc_decrypted = remove_padding(ecc_decrypted)
    start_time = time.time()
    rsa_decrypted = rsaDecrypt(rsa_privatekey, ecc_decrypted)
    print "imageDecrypt %s seconds" % (time.time() - start_time)
...
def creatImage(data, name):
    ...
    img.save("%s_encrypted.png"% name)
    print "%s_encrypted.png"% name
...
def recoverImage(data, name):
    with open("%s_decrypted.png" % name, "wb") as outfile:
        outfile.write(data)

    im = Image.open("%s_decrypted.png" % name)
    im.show()
